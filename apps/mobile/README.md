...
| A secure `wss` relay    | iOS ATS blocks plain `ws` — you need a `wss` relay multiaddr |
...
Copy `app.example.json` to `app.json` and set `expo.extra.relayMultiaddr` to a secure `wss` multiaddr.:

```jsonc
{
  "expo": {
    "extra": {
      // REQUIRED — must be a secure wss multiaddr (iOS ATS blocks ws).
      "relayMultiaddr": "/dns4/your-relay.example/tcp/443/wss/p2p/<relay-peer-id>",
...
identity screen will surface a "set a wss relay" error on join.
...
| Join fails with a "set a wss relay" message            | Set `expo.extra.relayMultiaddr` to a secure `wss` multiaddr.                                                            |
