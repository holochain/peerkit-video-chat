 ...
 *
 * A fresh controller is constructed per test with an injected mock platform, so
 * recovery state never leaks between cases. TURN is configured through the
 * controller dependencies so the relay-only rung is active.
 */

import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";

import { SharedMediaController } from "../src/media/peer-connection";
import type { MediaController } from "../src/media/types";
import {
  ACCEPTOR_GIVEUP_MS,
  DTLS_STALL_MS,
  RECOVERY_ATTEMPT_MS,
...
beforeEach(() => {
  ...
  resetPeers();
  h = makeHarness();
  ctrl = new SharedMediaController(
    {
...
describe("offer/answer setup", () => {
  ...
  it("methods work when destructured off the controller", async () => {
    // A consumer may do `const { initiateCall, closeAll } = controller` and call
    // them free-standing — methods must keep their `this` binding.
