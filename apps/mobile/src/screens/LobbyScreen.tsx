...
// A name is joinable only if it has at least one non-dash character after
// sanitization; "---" sanitizes cleanly but carries no actual room name.
function isJoinableRoomName(value: string): boolean {
  return sanitizeRoomName(value).replace(/-/g, "") !== "";
}
...
export function LobbyScreen({
  ...
  function submit(): void {
    if (!isJoinableRoomName(draft)) return;
    setDraft("");
    onJoin(sanitizeRoomName(draft));
  }
    ...
    <View style={styles.screen}>
      ...
      <View style={styles.joinRow}>
        ...
        <TextInput
          ...
          value={draft}
        />
        <Pressable disabled={!isJoinableRoomName(draft)} onPress={submit} style={styles.joinButton}>
