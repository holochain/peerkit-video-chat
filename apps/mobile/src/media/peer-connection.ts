/**
 * WebRTC media controller for the react-native app.
 *
 ...
 * and transceiver/`replaceTrack` camera toggling. It reaches for no react-native
 * global at import time — every platform binding (the WebRTC constructors,
 * `getUserMedia`, the speaking detector) is injected via {@link MediaPlatform}.
 * `platform.ts` builds the react-native platform and hands it here; the test suite
 * injects a mock platform instead, so the resilience logic runs under node without
 * a real WebRTC stack.
 */
...
export class SharedMediaController implements MediaController {
  ...
  async handleSignal(fromAgentId: string, signal: WebRtcSignal): Promise<void> {
    if (signal.kind === "offer") {
      ...
      if (existingPc !== undefined) {
        ...
        this.resetPeerMediaState(fromAgentId);
  ...
  private buildPeerConnection(agentId: string, relayOnly = false): RTCPeerConnection {
    ...
    pc.onconnectionstatechange = () => {
      ...
      if (state === "failed") {
        ...
        // Only tear down if this pc is still the live one. A full reconnect or a
        // different-fingerprint offer closes the old pc before the replacement is
        // stored; a delayed "closed" from the stale instance must not reap the
        // new connection.
        if (this.peers.get(agentId) === pc) this.closePeer(agentId);
      }
    };
  ...
  /**
   * Drop the per-peer inbound stream and speaking detector when the peer
   * connection is being replaced (full reconnect / different-fingerprint offer).
   * The replacement pc delivers tracks with fresh ids, so without this the old
   * aggregate stream keeps its now-ended tracks and ontrack appends new ones
   * beside them, while the detector stays bound to the dead pc's getStats. A
   * fresh stream and detector are rebuilt by ontrack when the new tracks arrive.
   */
  private resetPeerMediaState(agentId: string): void {
  ...
  private async fullReconnect(agentId: string, relayOnly = false): Promise<void> {
    ...
    this.pendingEoc.delete(agentId);
    this.videoSenders.delete(agentId);
    this.resetPeerMediaState(agentId);
    this.clearDtlsWatchdog(agentId);
    this.iceRestartAttempts.delete(agentId);
